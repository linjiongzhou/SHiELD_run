...
$y $m $d $h 0 0 
EOF
cat ${BUILD_AREA}/tables/diag_table_6species_cosp >> diag_table
...
########################################################################
    ...
    cd $dateDir
      ...
      if ( ! -d ${begindate}_cosp2d ) mkdir -p ${begindate}_cosp2d
      mv cosp2d*.nc* ${begindate}_cosp2d
      mv ${begindate}_cosp2d ../.
    ...
    #sbatch --export=source=$WORKDIR/history/${begindate}_cosp2d,destination=gfdl:$gfdl_archive/history/${begindate}_cosp3d,extension=tar,type=history --output=$HOME/STDOUT/%x.o%j $SEND_FILE
